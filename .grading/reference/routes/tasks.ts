...
// FIXED: Validates title
router.post('/', (req: Request, res: Response) => {
  const db = getDb();
  const { title, description, status, priority } = req.body;

  // FIXED: Validate title
  ...
  try {
    const stmt = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `);
    stmt.run(id, title.trim(), taskDescription, taskStatus, taskPriority);
...
// GET /api/tasks — FIXED: proper filtering, safe sorting, correct pagination
router.get('/', (req: Request, res: Response) => {
  ...
  // FIXED: Actually filter by status
  if (status) {
    whereClause = 'WHERE status = ?';
    params.push(status);
  }

  // FIXED: Whitelist sort fields to prevent SQL injection
  const allowedSortFields = ['title', 'priority', 'created_at', 'updated_at', 'status'];
...
// PUT /api/tasks/:id — FIXED: returns 404 when not found
router.put('/:id', (req: Request, res: Response) => {
  ...
  // FIXED: Return 404 if task doesn't exist
  if (!existing) {
    res.status(404).json({ error: 'Task not found' });
    return;
  }
  ...
  try {
    ...
    const task = db.prepare('SELECT * FROM tasks WHERE id = ?').get(req.params.id) as Task;
    res.json({ data: task });
  } catch (err: any) {
    res.status(400).json({ error: err.message });
  }
...
// DELETE /api/tasks/:id — FIXED: actually deletes
