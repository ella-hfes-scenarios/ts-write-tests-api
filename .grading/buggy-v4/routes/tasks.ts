// BUGGY-V4: DELETE doesn't actually delete (missing .run())
...
// BUG: prepares but never runs the delete
router.delete('/:id', (req: Request, res: Response) => {
  const db = getDb();
  const task = db.prepare('SELECT * FROM tasks WHERE id = ?').get(req.params.id);
  if (!task) { res.status(404).json({ error: 'Task not found' }); return; }
  db.prepare('DELETE FROM tasks WHERE id = ?'); // BUG: missing .run(req.params.id)
  res.status(204).send();
});
